...
st.sidebar.info(
    "Esta aplicación muetra un análisis del Indicador de Rendimiento Neto (IRN) a partir de un archivo XLSX que debes cargar. "
    "Selecciona los filtros para cada hoja de datos y observa los resultados en forma de tabla y gráfico de barras."
)

# Widget para que el usuario cargue el archivo XLSX
uploaded_file = st.sidebar.file_uploader("Cargar archivo XLSX", type=["xlsx"])

if uploaded_file is not None:
    # Obtener los nombres de las hojas del archivo XLSX cargado
    ...
    df1 = pd.read_excel(uploaded_file, sheet_name='IRN')
    df2 = pd.read_excel(uploaded_file, sheet_name='IRN_promedio')
    df3 = pd.read_excel(uploaded_file, sheet_name='IRN_ponderado')
